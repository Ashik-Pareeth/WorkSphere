...
public class LeaveRequestService {
        ...
        public LeaveRequest submitRequest(String username, UUID policyId, LocalDate startDate, LocalDate endDate,
                ...
                notificationService.send(
                                ...
                                "Your " + policy.getName() + " leave request from " + startDate + " to " + endDate
                ...
                if (employee.getManager() != null) {
                        notificationService.send(
                                        ...
                                        NotificationType.LEAVE_SUBMITTED,
                                        "New Leave Request from " + employee.getFirstName() + " "
                                                        + employee.getLastName(),
                                        employee.getFirstName() + " " + employee.getLastName() + " has requested "
                                                        + requestedDays + " day(s) of " + policy.getName() + " leave ("
        ...
        public LeaveRequest approveRequest(UUID requestId, String managerUsername, String comment) {
                ...
                notificationService.send(
                                ...
                                "Your leave request from " + request.getStartDate() + " to " + request.getEndDate()
                                                + " has been approved by " + manager.getFirstName() + " "
                                                + manager.getLastName()
                                                + (comment != null && !comment.isBlank() ? ". Note: " + comment : "."),
                                savedRequest.getId(),
                                "LeaveRequest");

                auditService.log("LeaveRequest", savedRequest.getId(), com.ucocs.worksphere.enums.AuditAction.UPDATED,
                                manager.getId(), "PENDING", "APPROVED");
        ...
        public LeaveRequest rejectRequest(UUID requestId, String managerUsername, String comment) {
                ...
                auditService.log("LeaveRequest", saved.getId(), com.ucocs.worksphere.enums.AuditAction.UPDATED,
                                manager.getId(), "PENDING", "REJECTED");
        ...
        public List<LeaveRequest> getApprovedLeaveBoard(String username, String requestedScope) {
                ...
                return switch (scope) {
                        case "GLOBAL" -> {
                                if (!isHrOrAdmin(viewer)) {
                                        throw new AccessDeniedException(
                                                        "Only HR or admins can view global leave records.");
                                }
                                yield leaveRequestRepository
                                                .findByStatusOrderByStartDateAsc(LeaveRequestStatus.APPROVED);
                        }
                        case "TEAM" -> {
                                if (!isManager(viewer)) {
                        ...
                        case "MY" -> leaveRequestRepository.findByEmployee_IdInAndStatusOrderByStartDateAsc(
                                        ...
                                        LeaveRequestStatus.APPROVED);
                        default ->
                                throw new IllegalArgumentException("Unsupported leave board scope: " + requestedScope);
                };
        }
        ...
        public LeaveRequest cancelRequest(UUID requestId, String username) {
                ...
                // NOTIFICATION: Confirm cancellation to the employee (if they didn't cancel it
                // themselves, e.g. HR cancelled it)
                if (!request.getEmployee().getId().equals(employee.getId())) {
                        notificationService.send(
                                        request.getEmployee().getId(),
                                        NotificationType.LEAVE_CANCELLED,
                                        "Leave Request Cancelled",
                                        "Your leave request from " + request.getStartDate() + " to "
                                                        + request.getEndDate() + " has been cancelled by "
        ...
        @Transactional
        public LeaveRequest updatePendingRequest(UUID requestId, String username,
                        LocalDate startDate, LocalDate endDate, double requestedDays, String reason) {
                LeaveRequest request = leaveRequestRepository.findById(requestId)
                                .orElseThrow(() -> new ResourceNotFoundException("Leave request not found"));
                Employee employee = employeeRepository.findByUserName(username)
                                .orElseThrow(() -> new ResourceNotFoundException("Employee not found"));
                ...
                LeaveRequest saved = leaveRequestRepository.save(request);
                auditService.log("LeaveRequest", saved.getId(), com.ucocs.worksphere.enums.AuditAction.UPDATED,
                                employee.getId(), "PENDING_EDIT", reason);
                return saved;
        }
}

...
import { fetchAllAssets, createAsset, assignAsset, returnAsset } from '../../api/hrApi';
import { getAllEmployees } from '../../api/employeeApi';
import { useAuth } from '../../hooks/useAuth';

const ASSET_TYPES = ['LAPTOP', 'MONITOR', 'KEYBOARD', 'PHONE', 'ACCESS_CARD', 'OTHER'];
...
const AssetDirectory = () => {
  ...
  const [newAsset, setNewAsset] = useState({
    type: 'LAPTOP', makeModel: '', serialNumber: '',
    purchaseDate: '', warrantyExpiry: '', condition: 'NEW', notes: '',
  });
    ...
    <div className="flex flex-col h-screen bg-white text-gray-900 font-sans w-full overflow-hidden">
      ...
      <header className="flex-none border-b border-gray-200 bg-white px-6 py-4 z-10">
        <div className="flex w-full items-center justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-gray-900">Asset Directory</h1>
            <p className="text-xs text-gray-500 mt-0.5">
              {assets.length} assets registered · {assigned.length} assigned · {unassigned.length} available
            </p>
          </div>
      ...
      <main className="flex-1 overflow-auto bg-gray-50/50 p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-gray-200 text-left">
                <th className="pb-3 pl-4 font-semibold text-gray-500 uppercase tracking-wider">Asset</th>
                <th className="pb-3 font-semibold text-gray-500 uppercase tracking-wider">Make / Model</th>
                <th className="pb-3 font-semibold text-gray-500 uppercase tracking-wider">Serial</th>
                <th className="pb-3 font-semibold text-gray-500 uppercase tracking-wider">Condition</th>
                <th className="pb-3 font-semibold text-gray-500 uppercase tracking-wider">Assigned To</th>
                <th className="pb-3 font-semibold text-gray-500 uppercase tracking-wider">Warranty</th>
                <th className="pb-3 pr-4 font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {assets.map((asset) => (
                <tr key={asset.id} className="border-b border-gray-100 hover:bg-gray-50/80 transition-colors">
                  ...
                  <td className="py-3 font-medium text-gray-800">{asset.makeModel}</td>
                  <td className="py-3 font-mono text-gray-500">{asset.serialNumber}</td>
                  <td className="py-3">
                  ...
                  <td className="py-3">
                    {asset.assignedEmployeeName ? (
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-medium text-gray-700">{asset.assignedEmployeeName}</span>
                      </div>
                    ) : (
                  ...
                  <td className="py-3">
                    {asset.warrantyExpiry ? (
                      <span className={`text-[10px] font-medium ${new Date(asset.warrantyExpiry) < new Date() ? 'text-red-500' : 'text-gray-500'}`}>
                        {new Date(asset.warrantyExpiry).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                      </span>
          ...
          {assets.length === 0 && (
            <div className="text-center py-12 text-sm text-gray-400">No assets found.</div>
          )}
        </div>
      </main>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowAddModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-200" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-gray-900 mb-4">Register New Asset</h2>
            <form onSubmit={handleCreate} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={newAsset.condition} onChange={(e) => setNewAsset(p => ({...p, condition: e.target.value}))}
                    className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none">
              ...
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Make / Model *</label>
                <input type="text" required value={newAsset.makeModel} onChange={(e) => setNewAsset(p => ({...p, makeModel: e.target.value}))}
                  placeholder="e.g. Dell XPS 15 9500"
                  className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
              </div>
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Serial Number *</label>
                <input type="text" required value={newAsset.serialNumber} onChange={(e) => setNewAsset(p => ({...p, serialNumber: e.target.value}))}
                  className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Purchase Date</label>
                  <input type="date" value={newAsset.purchaseDate} onChange={(e) => setNewAsset(p => ({...p, purchaseDate: e.target.value}))}
                    className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
                </div>
                <div>
                  <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Warranty Expiry</label>
                  <input type="date" value={newAsset.warrantyExpiry} onChange={(e) => setNewAsset(p => ({...p, warrantyExpiry: e.target.value}))}
                    className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Notes</label>
                <textarea value={newAsset.notes} onChange={(e) => setNewAsset(p => ({...p, notes: e.target.value}))} rows={2}
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit"
                  className="flex-1 px-4 py-2 text-xs font-semibold bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors">Register Asset</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {/* Assign Modal */}
      {showAssignModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowAssignModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-gray-200" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-gray-900 mb-4">Assign Asset</h2>
            <form onSubmit={handleAssign} className="space-y-3">
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Employee *</label>
                <select required value={assignData.employeeId} onChange={(e) => setAssignData(p => ({...p, employeeId: e.target.value}))}
              ...
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Notes</label>
                <textarea value={assignData.notes} onChange={(e) => setAssignData(p => ({...p, notes: e.target.value}))} rows={2}
                  className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none resize-none" />
              </div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowAssignModal(null)}
                  className="flex-1 px-4 py-2 text-xs font-semibold border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
                <button type="submit"
                  className="flex-1 px-4 py-2 text-xs font-semibold bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Assign</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {/* Return Modal */}
      {showReturnModal && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowReturnModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-gray-200" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-gray-900 mb-4">Return Asset</h2>
            <form onSubmit={handleReturn} className="space-y-3">
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Condition on Return *</label>
                <select value={returnData.condition} onChange={(e) => setReturnData(p => ({...p, condition: e.target.value}))}
                  className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none">
                  {CONDITIONS.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Notes</label>
                <textarea value={returnData.notes} onChange={(e) => setReturnData(p => ({...p, notes: e.target.value}))} rows={2}
                  className="w-full mt-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none resize-none" />
              </div>
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowReturnModal(null)}
                  className="flex-1 px-4 py-2 text-xs font-semibold border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
                <button type="submit"
                  className="flex-1 px-4 py-2 text-xs font-semibold bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors">Confirm Return</button>
              </div>
            </form>
          </div>
        </div>

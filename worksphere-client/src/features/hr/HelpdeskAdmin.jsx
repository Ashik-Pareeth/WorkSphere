...
import { fetchAllTickets, assignTicket, resolveTicket, addTicketComment } from '../../api/hrApi';
    ...
    <div className="flex flex-col h-screen bg-white text-gray-900 font-sans w-full overflow-hidden">
      <header className="flex-none border-b border-gray-200 bg-white px-6 py-4 z-10">
        <div className="flex w-full items-center justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-gray-900">Helpdesk Administration</h1>
            <p className="text-xs text-gray-500 mt-0.5">
              {tickets.length} total tickets · {getTicketsByStatus('OPEN').length} open · {getTicketsByStatus('IN_PROGRESS').length} in progress
            </p>
          </div>
        </div>
      </header>
              ...
              <div className="flex items-center justify-between mb-3 px-1">
                <div className="flex items-center gap-2">
                  <span className={`w-2.5 h-2.5 rounded-full ${
                    ...
                    status === 'PENDING_INFO' ? 'bg-purple-500' : 'bg-emerald-500'
                  }`} />
              ...
              <div className="flex-1 min-h-0 rounded-xl border bg-gray-100/40 border-gray-200/60 p-2 overflow-y-auto">
                {getTicketsByStatus(status).map((ticket) => (
                  ...
                  >
                    ...
                    <h4 className="text-sm font-semibold leading-snug mb-2 text-gray-900">{ticket.subject}</h4>

                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-base">{categoryIcon(ticket.category)}</span>
                      <span className="text-[10px] font-medium text-gray-500">{ticket.category.replace(/_/g, ' ')}</span>
                    </div>

                    <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                      <span className="text-[10px] text-gray-400">{ticket.raisedByName}</span>
                      <span className="text-[10px] text-gray-400">{daysSince(ticket.createdAt)}d ago</span>
                    </div>
                  </div>
                ))}
                {getTicketsByStatus(status).length === 0 && (
                  <div className="text-center py-8 text-[10px] text-gray-400">No tickets</div>
                )}
              </div>
            </div>
          ))}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-700">{categoryIcon(selectedTicket.category)} {selectedTicket.category.replace(/_/g, ' ')}</div>
                </div>
                <div>
                  <span className="text-[10px] font-semibold text-gray-400 uppercase">Raised By</span>
                  <div className="mt-1 text-xs text-gray-700">{selectedTicket.raisedByName}</div>
                </div>
              </div>
              ...
              {selectedTicket.status === 'OPEN' && (
                <div className="flex gap-2">
                  ...
                  >
                    <option value="" disabled>Select IT/HR/Admin Handler...</option>
                    {employees.map(emp => (
                  ...
                  <button onClick={() => handleAssign(selectedTicket.id)}
              ...
              <div>
                ...
                <div className="mt-2 space-y-3 max-h-60 overflow-y-auto">
                  {(selectedTicket.comments || []).map((c) => (
                    <div key={c.id} className={`p-3 rounded-lg border text-xs ${c.isInternal ? 'bg-amber-50/50 border-amber-100' : 'bg-white border-gray-100'}`}>
                      <div className="flex justify-between mb-1">
                        <span className="font-semibold text-gray-700">{c.authorName}</span>
                        <div className="flex items-center gap-1.5">
                          {c.isInternal && <span className="text-[9px] font-bold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded">INTERNAL</span>}
              ...
              {selectedTicket.status !== 'RESOLVED' && selectedTicket.status !== 'CLOSED' && (
                <div className="border-t border-gray-100 pt-4">
                  <span className="text-[10px] font-semibold text-gray-400 uppercase">Resolve Ticket</span>

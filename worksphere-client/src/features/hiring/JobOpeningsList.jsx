    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                ...
                <Button className="bg-blue-600 hover:bg-blue-700" onClick={() => setIsCreatingJob(true)}>
            ...
            {jobs.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {jobs.map((jobStats) => {
                        ...
                        return (
                        <Card key={job.id} className="bg-white dark:bg-gray-800 border overflow-hidden hover:shadow-lg transition-all duration-300">
                            <CardHeader className="border-b bg-gray-50/50 dark:bg-gray-900/50 pb-4">
                                ...
                                <p className="text-sm text-gray-500 mt-1">
                                    {job.department?.name || 'No Dept'} • {job.openSlots} Openings
                                </p>
                            </CardHeader>
                            <CardContent className="pt-4 flex justify-between items-center">
                                <div className="flex space-x-4 text-sm text-gray-500">
                                    <div>
                                        <span className="font-semibold text-gray-900 dark:text-gray-100">{jobStats.candidateCount}</span> Candidates
                                    </div>
                                    <div>
                                        <span className="font-semibold text-gray-900 dark:text-gray-100">{jobStats.interviewCount}</span> Interviews
                                    </div>
                                </div>
                                <Button 
                                    variant="outline" 

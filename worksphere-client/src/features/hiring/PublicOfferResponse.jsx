...
const PublicOfferResponse = () => {
    ...
    useEffect(() => {
        if (!token) {
            setError('Invalid or missing secure token. Please use the exact link from your email.');
            setLoading(false);
            return;
        }
    ...
    const handleAction = async (accept) => {
        if (!window.confirm(`Are you sure you want to ${accept ? 'ACCEPT' : 'DECLINE'} this offer? This action cannot be easily undone.`)) return;
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50 text-gray-500">Loading your secure offer...</div>;
                ...
                <div className="max-w-md w-full bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-xl text-center border border-gray-100 dark:border-gray-800">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-6">
                        {isAccepted 
                            ...
                            : "You have declined this offer. We wish you the best in your future endeavors."}
                    </p>
                    <Button onClick={() => window.location.href = 'https://worksphere.com'} className="w-full">
            ...
            <div className="max-w-3xl mx-auto space-y-8">
                <div className="text-center">
                    ...
                    <p className="mt-2 text-gray-500 dark:text-gray-400">
                        Please review the details below carefully before making your decision.
                    </p>
                </div>
                    ...
                    <div className="p-8 space-y-8">
                        ...
                        <div className="border-b border-gray-100 dark:border-gray-800 pb-6">
                            ...
                            <div className="prose dark:prose-invert max-w-none text-sm text-gray-600 dark:text-gray-300">
                                <p>
                                    We are thrilled to offer you the position of <strong>{offer.jobOpening?.title}</strong> in the <strong>{offer.jobOpening?.department?.name}</strong> department.
                                </p>
                                <p>
                                    Based on your excellent interviews and qualifications, we are confident you will be a great addition to our team. Please find the core compensation and logistics below.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-gray-800/50 p-6 rounded-xl border border-gray-100 dark:border-gray-700">
                            <div>
                                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Position</h4>
                                <p className="font-medium text-gray-900 dark:text-white">{offer.jobOpening?.title}</p>
                            </div>
                            <div>
                                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Department</h4>
                                <p className="font-medium text-gray-900 dark:text-white">{offer.jobOpening?.department?.name}</p>
                            </div>
                            <div>
                                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Annual Compensation</h4>
                                <p className="font-bold text-lg text-green-600 dark:text-green-400">
                                    ${offer.proposedSalary?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </p>
                            </div>
                            <div>
                                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Expected Start Date</h4>
                                <p className="font-medium text-gray-900 dark:text-white">
                                    {new Date(offer.joiningDate).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="pt-6 border-t border-gray-100 dark:border-gray-800">
                            <p className="text-sm text-gray-500 mb-6 text-center">
                                By accepting this offer, you agree to officially join our organization. You will immediately be provisioned with your employee accounts.
                            </p>

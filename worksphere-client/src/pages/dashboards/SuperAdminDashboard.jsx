...
import { Users, Shield, Settings, UserPlus, Building2, Calendar, FileText, ChevronRight } from 'lucide-react';
...
export default function SuperAdminDashboard() {
    ...
    const fetchSystemStats = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          const pending = employees.filter(e => e.status === 'PENDING' || e.employeeStatus === 'PENDING');
          setPendingOnboardings(pending.slice(0, 5));

          const roles = rolesRes?.data || [];
          setRolesCount(roles.length || 5); // Fallback to 5 standard roles
        }
      } catch (error) {
  ...
  const ConfigLink = ({ to, icon, title, desc }) => (
    <Link to={to} className="flex items-center p-4 bg-white border border-gray-100 rounded-xl hover:border-blue-300 hover:shadow-md transition-all group">
      ...
      <div className="ml-4 flex-1">
        <h3 className="text-sm font-bold text-gray-900 group-hover:text-blue-700">{title}</h3>
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 text-gray-900 w-full overflow-y-auto">
      <main className="flex-1 p-8 w-full max-w-7xl mx-auto space-y-8">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">System Administration</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          />
          <div className="lg:col-span-1 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50 flex items-center justify-center text-xs font-semibold text-gray-400 uppercase tracking-widest">
            Phase 3 Slot
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           ...
           <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 <ConfigLink to="/departments" icon={<Building2 />} title="Departments" desc="Manage org segments" />
                 <ConfigLink to="/role-management" icon={<Shield />} title="Role Allocation" desc="Assign user permissions" />
                 <ConfigLink to="/work-schedules" icon={<Calendar />} title="Work Schedules" desc="Configure shift timings" />
                 <ConfigLink to="/leave-policies" icon={<FileText />} title="Leave Policies" desc="Adjust accrual rules" />
              </div>
           </div>
           ...
           <div className="lg:col-span-1">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                 {loading ? (
                    ...
                    <div className="p-8 text-center text-sm text-gray-500">No pending accounts. All clear!</div>
                 ) : (
                             ...
                             <div className="flex items-center gap-3">
                                ...
                                <div>
                                   ...
                                   <p className="text-[11px] text-gray-500 uppercase tracking-widest">
                                      {emp.createdAt ? new Date(emp.createdAt).toLocaleDateString() : 'Awaiting Activation'}
                                   </p>
                                </div>
                             </div>
                          </div>
